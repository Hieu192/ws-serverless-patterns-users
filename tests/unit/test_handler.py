...
def test_get_list_of_users():
    with my_test_environment():
        ...
        # Code đã sửa trả về {'items': [...]} với pagination
...
def test_get_single_user_wrong_id():
    with my_test_environment():
        ...
        ret = users.lambda_handler(apigw_event, '')
        # Code đã sửa trả về {} với status 200 (giống workshop gốc)
        assert ret['statusCode'] == 200
        assert json.loads(ret['body']) == {}

@patch('uuid.uuid4', mock_uuid)  # Code đã đổi từ uuid1 sang uuid4
...
def test_add_user():
    with my_test_environment():
        ...
        ret = users.lambda_handler(apigw_event, '')
        # Code đã sửa trả về 201 (Created) thay vì 200
        assert ret['statusCode'] == 201
        data = json.loads(ret['body'])
        assert data['userid'] == UUID_MOCK_VALUE_NEW_USER
        # Code dùng timezone.utc nên format khác
        assert data['timestamp'].startswith('2001-01-01T00:00:00')
...
def test_delete_user():
    with my_test_environment():
        ...
        ret = users.lambda_handler(apigw_event, '')
        assert ret['statusCode'] == 200
        # Code đã sửa trả về message thay vì {}
        data = json.loads(ret['body'])
